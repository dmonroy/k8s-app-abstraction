...
def test_stack_install():
    ...
    with ephemeral_rollout(chart):
        ...
        assert k8s_resources["Deployment/my-stack-a-deploy"].spec.replicas == 2
        assert (
            k8s_resources["Deployment/my-stack-a-deploy"]
            .spec.template.spec.containers[0]
            .image
            == "bar"
        )
        assert k8s_resources["StatefulSet/my-stack-a-statefulset"].spec.replicas == 1
        assert (
            k8s_resources["StatefulSet/my-stack-a-statefulset"]
            .spec.template.spec.containers[0]
            .image
            == "bar"
        )
        assert (
            k8s_resources["DaemonSet/my-stack-a-daemonset"]
            .spec.template.spec.containers[0]
            .image
            == "bar"
        )
